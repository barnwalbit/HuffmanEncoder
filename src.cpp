...
void buildHuffmanTree(string text){
	// counting frequency of appearance of each character and store it in a map
	unordered_map<char, int> freq;
	for(char ch: text) freq[ch]++;

	// Creating a priority queue to store live nodes of Huffman tree;
	...
	while(pq.size()!=1){
		...
		// Creating a new internal node with these two nodes as children and with frequency equal to the sum of the two nodes' frequencies. Add the new node to the priority queue.
